...
// Keybindings are generated from Hyprland's live binding cache. Replace the
// previous generated batch without disturbing static menu rows or app rows.
function mergeKeybindingRows(items, itemOrder, keybindingRows) {
  var source = items || ({})
  var order = Array.isArray(itemOrder) ? itemOrder : []
  var rows = Array.isArray(keybindingRows) ? keybindingRows : []
  var nextItems = ({})
  var nextOrder = []
  ...
  for (var i = 0; i < order.length; i++) {
    var id = order[i]
    var existing = source[id]
    if (!existing || existing.generatedSource === "keybindings") continue
    nextItems[id] = existing
    nextOrder.push(id)
  }
  ...
  for (var j = 0; j < rows.length; j++) {
    var row = rows[j]
    if (!row || !row.id || nextItems[row.id]) continue
    row.generatedSource = "keybindings"
    row.order = nextOrder.length
    nextItems[row.id] = row
    nextOrder.push(row.id)
  }
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    mergeKeybindingRows: mergeKeybindingRows,
