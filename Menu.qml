...
Item {
  ...
  readonly property string pluginDir: root.manifest && root.manifest.__sourceDir
    ...
    : Quickshell.env("HOME") + "/.config/omarchy/plugins/io.github.alivault.omarchy-unified-launcher"
  ...
  GlobalShortcut {
    appid: "io.github.alivault.omarchy-unified-launcher"
    name: "toggle"
    description: "Toggle Unified Launcher"
    onPressed: if (root.shell) root.shell.toggle(
      (root.manifest && root.manifest.id) || "io.github.alivault.omarchy-unified-launcher",
  ...
  function debugVisibleRows() {
    ...
    for (var i = 0; i < Math.min(displayModel.count, 12); i++) {
      ...
      rows.push({ label: row.label, shortcut: row.shortcut, kind: row.kind, detail: row.detail })
    }
  ...
  property string keybindingRecordsRaw: ""
  property var keybindingRows: []
  ...
  function refreshKeybindings() {
    if (!keybindingsProc.running) keybindingsProc.running = true
  }

  function parseKeybindingRecord(line, index) {
    var fields = String(line || "").split("\t")
    var display = fields.shift() || ""
    var dispatcher = fields.shift() || ""
    var argument = fields.join("\t")
    var arrow = display.indexOf("→")
    var shortcut = arrow >= 0 ? display.substring(0, arrow).trim() : display.trim()
    var label = arrow >= 0 ? display.substring(arrow + 1).trim() : shortcut
    return {
      id: "learn.keybindings.binding-" + index,
      parent: "learn.keybindings",
      kind: "keybinding",
      icon: "",
      iconFont: "",
      label: label,
      ...
      shortcut: shortcut,
      action: Util.shellQuote(root.pluginDir + "/dispatch.sh") + " "
        + Util.shellQuote(dispatcher) + " " + Util.shellQuote(argument),
      provider: "",
      aliases: [],
      when: "",
      checked: ""
  ...
  function loadKeybindingRecords(raw) {
    var nextRaw = String(raw || "").trim()
    if (!nextRaw || nextRaw === root.keybindingRecordsRaw) return

    var rows = []
    var lines = nextRaw.split("\n")
    for (var i = 0; i < lines.length; i++) {
      if (lines[i].trim()) rows.push(root.parseKeybindingRecord(lines[i], i))
    }

    root.keybindingRecordsRaw = nextRaw
    root.keybindingRows = rows
  ...
  function restoreMenuState(state) {
    var previous = state || ({})
    var id = root.item(previous.id) ? previous.id : "root"
    panel.freezeCardTop()
    root.activeMenu = id
    root.filterText = String(previous.filterText || "")
    root.selectedIndex = Math.max(0, Number(previous.selectedIndex || 0))
    root.cursorActive = true
    root.disarmPointer()
    root.rebuildDisplay()
    if (root.filterText.trim()) root.loadProvidersForSearch()
    else root.loadProviderForMenu(id)

    var savedContentY = Number(previous.contentY || 0)
    Qt.callLater(function() {
      var maxY = Math.max(resultList.originY, resultList.originY + resultList.contentHeight - resultList.height)
      resultList.contentY = Math.max(resultList.originY, Math.min(savedContentY, maxY))
      root.revealCursor()
    })
  }
  ...
  function goBack() {
    ...
    if (root.navStack.length > 0) {
      ...
      if (typeof previous === "string") root.setActiveMenu(previous, false)
      else root.restoreMenuState(previous)
      return true
    }
  ...
  Process {
    ...
    command: [
      "bash", "-c",
      "\"$0/bin/omarchy-menu-keybindings\" --print >/dev/null || exit; "
        + "cache_dir=\"${XDG_CACHE_HOME:-$HOME/.cache}/omarchy\"; "
        + "cache=$(find \"$cache_dir\" -maxdepth 1 -type f -name 'keybindings-*.records' -printf '%T@ %p\\n' 2>/dev/null | sort -nr | head -1 | cut -d' ' -f2-); "
        + "[[ -n $cache ]] && cat \"$cache\"",
      root.omarchyPath
    ]
    ...
    onExited: function(exitCode) {
      if (exitCode === 0) root.loadKeybindingRecords(keybindingsOutput.text)
    }
  }

  Component.onCompleted: root.refreshKeybindings()
        ...
        Item {
          ...
          ListView {
            ...
            delegate: BorderSurface {
              ...
              Row {
                id: trail
                width: shortcutText.width + arrowText.width
                anchors.right: parent.right
